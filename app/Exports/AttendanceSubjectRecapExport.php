...
class AttendanceSubjectRecapExport implements FromCollection, WithHeadings, ShouldAutoSize, WithDrawings, WithCharts, WithStyles, WithTitle, \Maatwebsite\Excel\Concerns\WithCustomStartCell
{
    ...
    public function charts()
    {
        ...
        // Persentase Kehadiran (%) row is at index: 7 (headings) + S (students) + 6 (summary rows) = 13 + S
        $pctRow = 7 + $S + 6;
        ...
        $title = new ChartTitle('Grafik Persentase Kehadiran Mapel Harian (%)');
        
        $chart = new Chart(
            'attendance_subject_chart',
            $title,
            $legend,
            $plotArea,
            true,
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('C3')->getFont()->setBold(true)->setSize(9)->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('4F46E5'));
        $sheet->getStyle('C4')->getFont()->setItalic(true)->setSize(9)->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('64748B'));
        $sheet->getStyle('C5')->getFont()->setSize(9)->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('64748B'));
        ...
        $sheet->getStyle('A8:B' . $endRow)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);
        $otherRange = 'C8:' . $lastColLetter . $endRow;
        $sheet->getStyle($otherRange)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
        
        $borderRange = 'A7:' . $lastColLetter . $endRow;
