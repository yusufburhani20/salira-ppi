...
export default function MyStudentsIndex({ classes }: any) {
    ...
    const [selectedClassId, setSelectedClassId] = useState<string>(classes.length === 1 ? classes[0].id.toString() : '');
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-slate-800 dark:text-slate-200">Siswa Wali Kelas</h2>}>
                ...
                {classes.length > 1 && (
                    <div className="w-full sm:w-64 flex-shrink-0">
                        <select
                            ...
                            className="w-full rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 py-2.5 px-3"
                        >
            ...
            <div className="space-y-6">
                ...
                ) : (!selectedClassId && classes.length > 1) ? (
                    <Card>
                        <div className="text-center py-10">
                            <svg className="w-12 h-12 text-indigo-400 mx-auto mb-3 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
                            <p className="text-slate-500 font-medium">Silakan pilih kelas terlebih dahulu</p>
                            <p className="text-sm text-slate-400 mt-1">Pilih kelas pada menu dropdown di atas untuk melihat daftar siswa.</p>
                        </div>
                    </Card>
                ) : (
                    classes.map((cls: any) => {
                        if (selectedClassId && cls.id.toString() !== selectedClassId) return null;
