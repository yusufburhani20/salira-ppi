...
export default function AgendaEdit({ agenda, classes, subjects = [], lesson_hours = [] }: { agenda: any, classes: any[], subjects: any[], lesson_hours: any }) {
    ...
    const getSlotsForDate = (dateStr: string) => {
        ...
        return lesson_hours[dayName] || [];
    };

    const [slotsForDay, setSlotsForDay] = useState<any[]>(() => {
        return getSlotsForDate(agenda.date ? agenda.date.substring(0, 10) : '');
    });
    ...
    const [selectedSlots, setSelectedSlots] = useState<string[]>(() => {
        ...
        const parsedLabels = labelsPart.split(',').map((lbl: string) => lbl.trim()).filter(Boolean);

        const initialSlots = getSlotsForDate(agenda.date ? agenda.date.substring(0, 10) : '');
        const allLabelsExist = parsedLabels.length > 0 && parsedLabels.every((label: string) => 
            initialSlots.some((s: any) => s.label === label)
        );

        return allLabelsExist ? parsedLabels : [];
    });

    const prevDateRef = useRef(data.date);

    useEffect(() => {
        if (data.date !== prevDateRef.current) {
            prevDateRef.current = data.date;
