...
export default function AgendaCreate({ classes, subjects = [] }: { classes: any[], subjects: any[] }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        activities: '',
        student_tasks: '',
        attendance: [] as { student_id: number; status: string; notes?: string }[],
    });
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('teacher.agendas.store'));
    };

    return (
        <AuthenticatedLayout
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Buat Jurnal Baru</h2>}
        >
