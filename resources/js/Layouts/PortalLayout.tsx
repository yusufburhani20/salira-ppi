...
export default function PortalLayout({ header, children }: PropsWithChildren<{ header?: ReactNode }>) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
            ...
            >
                {/* Logo Area */}
                <div className="flex h-16 items-center justify-between px-5 border-b border-slate-100 flex-shrink-0 bg-blue-600">
                    <Link href={route('portal.dashboard')} className="flex items-center gap-2" onClick={() => setSidebarOpen(false)}>
                        <img src="/images/Salira.png" alt="SALIRA Logo" className="h-7 w-auto brightness-0 invert" />
                        <span className="text-xl font-bold border-white text-white">
                            PORTAL
                        </span>
                    </Link>
                    <button
                ...
                <nav className="flex-1 overflow-y-auto py-4">
                    {navGroups.map((group) => (
                        <div key={group.group} className="mb-2">
                            <p className="px-5 pt-2 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                {group.group}
                            </p>
                            <div className="px-3 space-y-1">
                                {group.items.map((item) => (
                                    ...
                                    >
                                        <span className={`flex-shrink-0 ${item.active ? 'text-blue-500' : 'text-slate-400'}`}>
                                            {item.icon}
                                        </span>
            ...
            <div className="flex flex-col min-h-screen lg:pl-64">
                    ...
                    <header className="sticky top-0 z-30 h-16 flex items-center gap-3 px-4 sm:px-6 bg-white/95 dark:bg-slate-900/95 backdrop-blur border-b border-slate-200 dark:border-slate-700/60 shadow-sm">
                        ...
                        <div className="lg:hidden font-bold text-slate-800 dark:text-slate-200 text-lg">PORTAL SALIRA</div>
